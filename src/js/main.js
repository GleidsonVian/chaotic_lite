class GameEngine {
    ...
    setupBoard() {
        // Preenche o tabuleiro clonando cartas para garantir exércitos cheios
        let p1Deck = [];
        let p2Deck = [];
        
        for(let i = 0; i < 6; i++) {
            p1Deck.push(this.cloneCard(this.cards[i % this.cards.length]));
            // Desloca o index para o oponente ter posições ligeiramente diferentes
            p2Deck.push(this.cloneCard(this.cards[(i + 2) % this.cards.length])); 
        }
